...
> Data-Cleaner - Open source tool by AetherCodeHQ

`cleaning` `cli` `data` `etl` `golang`

## What is it?

**Data Cleaner** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Data-Cleaner.git
cd Data-Cleaner
go build -o Data-Cleaner .
```
...
```bash
# Basic usage
./Data-Cleaner --path ./target

# With options
./Data-Cleaner --path ./target --format json --output report.json

# Verbose mode
./Data-Cleaner --path ./target --verbose
```
...
### Basic scan
```bash
./Data-Cleaner --path ./my-project
```

### JSON report
```bash
./Data-Cleaner --path ./my-project --format json --output report.json
```
...
- name: Run Data Cleaner
  run: ./Data-Cleaner --path . --format json --output report.json
```
